...
def extract(alembic_path, object_name, piece_attrib, rest_frame, anim_frame, orient_threshold, unpack):
    subnet = create_main_subnet(hou.node('/obj'), '{0}_extract_animation'.format(object_name))
...
def create_collisions(dop_path, object_name):
    subnet = create_main_subnet(hou.node(dop_path), '{0}_collisions'.format(object_name))
